...
export default {
  ...
  data () {
    return {
      ...
      filterText: '',
      urlApi: process.env.URL_API_BASE + '/secao',
      fields: [
        {
          name: 'sala_descricao',
          title: 'Sala',
          sortField: 'sala_descricao'
        },
        {
          name: 'filme_nome',
          title: 'Filme',
          sortField: 'filme_nome'
        },
        {
          name: 'enabled',
          title: 'Presença',
      ...
      sortOrder: [
        { field: 'sala_descricao', direction: 'asc' },
        { field: 'filme_nome', direction: 'asc' },
  ...
  methods: {
    ...
    novo () {
      this.$router.push('/secao/novo')
    },
    editar (rowData) {
      this.$router.push('/secao/' + rowData.id_filme + '/' + rowData.id_sala + '/' + '/e')
    },
    view (rowData) {
      this.$router.push('/secao/' + rowData.id_filme + '/' + rowData.id_sala)
    },
    excluir (rowData) {
      SecaoActions.excluir(rowData.id_filme + '/' + rowData.id_sala)
    },
    async gerarExcel () {
      let res = await SecaoActions.listar(this.status)
      return res
    }
  }
}
