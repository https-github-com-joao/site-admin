    ...
    <div class="box-header">
      ...
      <div class="col-6 col-md-6 col-sm-4 col-xs-4 div-btn-novo">
        ...
        <export-excel
          ...
          :fetch = "gerarExcel"
          type = "xls"
          worksheet = "Equipamentos"
          name = "zanzar-equipamentos.xls"
...
export default {
  ...
  data () {
    return {
      css: VuetableCss,
      listaExcelFields: {
        'Codigo': 'cd_equipamento',
        'Codigo Player': 'cd_player',
        'Descricao': 'descricao',
        'Numero Serial': 'numero_serial',
        'Numero do Frame': 'mac_address',
        'Status': 'status_desc',
        'Local Atual': 'local_atual_desc',
        'Criado em': 'createdAt',
        'Alterado em': 'updatedAt'
      },
      filterText: '',
      urlApi: process.env.URL_API_BASE + '/equipamento' + ((this.status !== null && this.status !== undefined) ? ('/listar-por-status/' + this.status) : ''),
      fields: [
        {
          name: 'cd_equipamento',
          title: 'Código',
          sortField: 'cd_equipamento'
        },
        {
          name: 'cd_player',
          title: 'Código Player',
        ...
        },
        {
          name: 'numero_serial',
          title: 'Serial',
          sortField: 'numero_serial'
        },
        {
          name: 'mac_address',
          title: 'Número do Frame',
          sortField: 'mac_address'
        },
        {
          name: '__slot:status_desc',
          title: 'Status',
          sortField: 'idc_status'
        },
        {
          name: 'local_atual_desc',
          title: 'Local Atual',
          sortField: 'local_atual_desc'
        },
        '__slot:actions'
      ],
      sortOrder: [
        { field: 'cd_equipamento', direction: 'asc' }
      ],
      moreParams: {}
    }
  },
  ...
  watch: {
    status: function (val) {
      if (this.status) {
        this.$parent.$parent.$data.description = this.titulo
        this.urlApi = process.env.URL_API_BASE + '/equipamento/listar-por-status/' + this.status
      } else {
        this.$parent.$parent.$data.description = 'Listar'
        this.urlApi = process.env.URL_API_BASE + '/equipamento'
      }
    }
  },
  methods: {
    ...
    novo () {
      this.$router.push('/equipamentos/novo')
    },
    editar (rowData) {
      this.$router.push('/equipamentos/' + rowData.cd_equipamento + '/e')
    },
    view (rowData) {
      this.$router.push('/equipamentos/' + rowData.cd_equipamento)
    },
    excluir (rowData) {
    },
    async gerarExcel () {
      let res = await EquipamentoActions.listarEquipamento(this.status)
      return res
    }
  }
}
