...
export default {
  ...
  data () {
    return {
      ...
      filterText: '',
      urlApi: process.env.URL_API_BASE + '/sala',
      fields: [
        {
          name: 'id',
          title: 'Código',
  ...
  methods: {
    ...
    novo () {
      this.$router.push('/sala/novo')
    },
    editar (rowData) {
      this.$router.push('/sala/' + rowData.id + '/e')
    },
    view (rowData) {
      this.$router.push('/sala/' + rowData.id)
    },
    excluir (rowData) {
      this.$router.push('/sala/' + rowData.id)
    },
    async gerarExcel () {
      let res = await SalaActions.listar(this.status)
      return res
    }
  }
}
